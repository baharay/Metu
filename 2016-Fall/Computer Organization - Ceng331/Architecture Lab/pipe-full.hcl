...
# Bahar Aydemir 
# 
# I have only implemented iaddq instruction. It gave me the opportunity to combine irmovq  and addq #instructions. Lots of cycles gained from this operation.
# After the detailed examination of irmovq and Opq instructions, I have constructed the following #computation steps :
# 1- IIADDQ should be in valid instructions obviously.
# 2- It needs the register id and a constant word to add the constant to the register.
# 3- In decode stage, it needs the register B for both source and destination.
# 4- In execute stage first input to the ALU is the constant number so it should be val_C. The second #input to ALU is the register B, so it should be val_B. Moreover, the condition code should be set to #know whether an overflow occured or not. (It also gives an insight about the sign of the sum through #ZF and negative flag.) It became very useful with conditional jumps.
# 5- Despite the Opq instructions, IIADDQ uses ALUADD for the ALU function like the other #instructions.
####################################################################
...
bool set_cc = (E_icode == IIADDQ  || E_icode == IOPQ) && 
